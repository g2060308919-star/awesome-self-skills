...
Declare required view kinds by semantic responsibility, not keyword matching. Input acceptance ranges differ from constraints on generated outputs; an output duration or ratio does not by itself require an input-domain or timing view. Read the view-selection distinctions in `behavior-views.md` before freezing this declaration. A mismatch between a known rule and an elected model is a generation repair, never new product uncertainty.
...
Every span also requires `review_basis` with `reviewer`, `method`, and `evidence`: identify who reviewed it, how the source was read, and the actual content supporting the disposition. Review the source independently of the extracted Claim list. A closed span partition proves declared accounting only; it does not prove natural-language completeness or that a non-normative disposition is correct.

Inventory discovered non-text assets in required `source_assets` (use `[]` only after reviewing the source for assets). Each closed record contains `asset_id`, `source_id`, `locator_id`, `uri`, `status` (`reviewed`, `unread`, or `unavailable`), `classification`, and `review_basis`. Use an exact text-range locator for each Markdown/HTML reference, or a typed source locator for embedded document assets. Read the actual screenshot, diagram, prototype, table image, or other asset with an available reading capability; link text alone is not asset content. Record its extracted observations in the review evidence and atomic direct Claims bound to the asset locator. A reviewed normative or uncertain asset needs accepted direct evidence. Only an actually reviewed decoration may be non-normative. An unread/unavailable asset retains an honest inability explanation and requires source revision; never assert review or omit an asset to pass. The compiler performs no asset fetching, and its explicit-reference syntax guard is not a universal document parser. The Adapter owns discovering embedded and unsupported-format assets as well.
...
Each Fact Ledger entry requires `required_view_kinds` and nonblank `view_review_basis`. Independently inspect the source-defined behavior and record every required view kind from `flow`, `decision`, `state`, `input-domain`, `role`, `timing`, and `integration`, with the source-grounded reason for that selection (or for an empty list). This review precedes custom-responsibility naming. Do not remove a timing/state/input/integration requirement merely by calling its responsibility a decision or flow. These declarations are accountable semantic review, not compiler inference from keywords.
