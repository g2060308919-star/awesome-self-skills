...
- Machine contract: the eleven closed `v5-*.schema.json` files and matching
...
| --- | --- | --- | --- |
| C01 atomic requirement decomposition | `v5-normative-rule-inventory`, `v5-stable-id-preimage-registry`, `v5-canonical-array-manifest` | `compileSemanticSeed`, `applyEvidenceClaimReviews`, `stableV5Id` | `F-C01-atomicity.*`; `semantic-seed.test.mjs` |
| C02 ambiguity isolation | `v5-normative-rule-inventory`, `v5-answer-constraint-registry` | `compileSemanticSeed`, `applyEvidenceClaimReviews`, question-part compilation | `F-C02-ambiguity.*`; `semantic-seed.test.mjs`, `clarification.test.mjs` |
| C03 terminology consistency | `v5-normative-rule-inventory`, `v5-stable-id-preimage-registry` | compiler-owned term candidates, review reduction, stable term identity | `F-C03-terminology.*`; `semantic-seed.test.mjs` |
...
| C05 partial-answer preservation | `v5-answer-constraint-registry`, `v5-clarification-control-registry` | `parseClarificationResponse`, `reduceClarificationPreview` | `F-C05-partial.*`; `clarification.test.mjs` |
| C06 question-part FSM | `v5-fsm-registry`, `v5-answer-constraint-registry` | question-part state reduction and `advanceV5Run` dispatch | `F-C06-question-state.*`; `clarification.test.mjs`, `fsm-runtime.test.mjs` |
| C07 two-phase preview/confirm | `v5-fsm-registry`, `v5-reply-contracts.generated`, `v5-clarification-control-registry` | `compileClarificationPreview`, confirmation-token and stale-preview validation | `F-C07-two-phase.*`; `clarification.test.mjs`, `fsm-runtime.test.mjs` |
...
| C09 field correspondence / Behavior gaps | `v5-normative-rule-inventory`, `v5-stable-id-preimage-registry` | `validateBehaviorViews`, `compileBehaviorGapProposals`, `applyBehaviorGapReviews` | `F-C09-field-correspondence.*`; `behavior-contracts.test.mjs`, `behavior-gaps.test.mjs` |
...
| C11 domain contract | `v5-normative-rule-inventory`, `v5-canonical-array-manifest` | domain/cross-field contract validation and Case projection | `F-C11-domain.*`; `behavior-contracts.test.mjs`, `case-output.test.mjs` |
...
separately in `2026-09-11-v5-release-gates.md`.
