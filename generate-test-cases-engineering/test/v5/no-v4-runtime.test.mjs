...
test('production source and installed schemas expose one V5 workflow only', async () => {
  const sourceFiles = (await readdir(new URL('src/', root), { withFileTypes: true }))
    .filter((/** @type {any} */ entry) => entry.isFile())
    .map((/** @type {any} */ entry) => entry.name)
    .sort();
  ...
  assert.ok(schemaFiles.every((/** @type {string} */ file) => file.startsWith('v5-')));
  ...
  assert.deepEqual(manifest.schemas.map((/** @type {Record<string,any>} */ row) => row.file), schemaFiles);
});
...
test('public source, built runner, build, and package scripts contain no legacy operational dispatch', async () => {
  ...
  const forbidden = /advanceStrict|createV4|constructV4|stageV4|migrate-v3|detectV4Run|compiler_version[^\n]{0,40}["']0\.[45]\.0["']/u;
