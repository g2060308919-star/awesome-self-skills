...
test('all 51 FSM outcomes have one unique trigger and one generated reply contract', () => {
  ...
  assert.equal(contracts.replyContracts.rows.filter((/** @type {Record<string,any>} */ row) => row.source.kind === 'fsm_outcome').length, 51);
  for (const expected of contracts.fsmRegistry.outcomes) {
    ...
    const rows = contracts.replyContracts.rows.filter((/** @type {Record<string,any>} */ row) => row.source.kind === 'fsm_outcome' && row.source.outcome_id === expected.outcome_id);
...
test('runtime dispatches clarification preview and commit through registry outcomes', async () => {
  ...
  const evidenceSelector = review.available_actions.find((/** @type {Record<string,any>} */ selector) => selector.capability.kind === 'submit_artifact');
  ...
  const previewSelector = clarification.available_actions.find((/** @type {Record<string,any>} */ selector) => selector.capability.kind === 'preview_clarification_response');
  ...
  const commitSelector = preview.available_actions.find((/** @type {Record<string,any>} */ selector) => selector.capability.kind === 'commit_clarification_response');
...
test('public runtime completes source, evidence, behavior, Case, and canonical delivery', async () => {
  ...
  const decompositionReviews = /** @type {any[]} */ ([]);
  for (const candidate of seed.normative_units.flatMap((/** @type {Record<string,any>} */ unit) => unit.outcome_candidates)) {
    const candidateClaims = candidate.required_observation_slot_digests.map((/** @type {string} */ slot, /** @type {number} */ index) => ({ claim_client_key: `claim-${claims.length + index}`, primary_outcome_signature: { ...candidate.atom_signature, primary_observation_slot_digest: slot }, observation_slot_digests: [slot] }));
    claims.push(...candidateClaims);
    decompositionReviews.push({ seed_digest: seed.seed_digest, candidate_id: candidate.candidate_id, disposition: candidateClaims.length === 1 ? { kind: 'single_claim', claim_client_key: candidateClaims[0].claim_client_key } : { kind: 'split_claims', claim_client_keys: candidateClaims.map((/** @type {Record<string,any>} */ claim) => claim.claim_client_key) } });
  }
  const evidenceSelector = review.available_actions.find((/** @type {Record<string,any>} */ selector) => selector.capability.kind === 'submit_artifact');
  const behavior = await advanceV5Run(created.run_directory, { idempotency_key: 'evidence-delivery', action: { kind: 'submit_artifact', action_token: evidenceSelector.action_token, artifact_kind: 'evidence_claims', artifact: { semantic_review_seed_digest: seed.seed_digest, claims, semantic_gaps: [], decomposition_reviews: decompositionReviews, ambiguity_reviews: seed.ambiguity_candidates.map((/** @type {Record<string,any>} */ candidate) => ({ seed_digest: seed.seed_digest, candidate_id: candidate.candidate_id, disposition: { kind: 'resolved_by_claims', claim_client_keys: [claims[0].claim_client_key] } })), entity_resolutions: [] } } });
  assert.equal(behavior.obligation, 'provide_behavior_views');
  const evidenceBindings = new Map(behavior.commit_receipt.client_key_bindings.map((/** @type {Record<string,any>} */ binding) => [binding.client_key, binding.stable_id]));
  ...
  const pointByClaimId = new Map(obligations.outcomes.map((/** @type {Record<string,any>} */ outcome) => [outcome.claim_ids[0], obligations.formal_test_points.find((/** @type {Record<string,any>} */ point) => point.outcome_id === outcome.outcome_id).formal_test_point_id]));
  const oracleContracts = behaviorSeed.required_contracts.map((/** @type {Record<string,any>} */ row, /** @type {number} */ index) => ({
  ...
  const riskReviews = behaviorSeed.risk_review_module_ids.flatMap((/** @type {string} */ moduleRef) => riskKinds.map((riskKind) => ({ review_client_key: `risk-${moduleRef}-${riskKind}`, module_ref: moduleRef, risk_kind: riskKind, risk_signal_status: 'no_signal', review_basis: [{ kind: 'claim', claim_id: evidenceBindings.get(claims[0].claim_client_key) }] })));
  const behaviorSelector = behavior.available_actions.find((/** @type {Record<string,any>} */ selector) => selector.capability.kind === 'submit_artifact');
  const behaviorArtifact = {
    ...
    behavior_contract_reviews: behaviorSeed.required_contracts.map((/** @type {Record<string,any>} */ row, /** @type {number} */ index) => ({ seed_digest: behaviorSeed.seed_digest, required_contract_key: row.required_contract_key, disposition: { kind: 'formal', contract_client_keys: [oracleContracts[index].oracle_contract_client_key] } })),
  ...
  const malformedField = /** @type {any} */ (structuredClone(behaviorArtifact));
  ...
  const undecidable = /** @type {any} */ (structuredClone(behaviorArtifact));
  ...
  for (const localKey of [...claims.map((/** @type {Record<string,any>} */ claim) => claim.claim_client_key), ...oracleContracts.map((/** @type {Record<string,any>} */ contract) => contract.oracle_contract_client_key), ...riskReviews.map((/** @type {Record<string,any>} */ review) => review.review_client_key)]) {
  ...
  const behaviorBindings = new Map(caseWork.commit_receipt.client_key_bindings.map((/** @type {Record<string,any>} */ binding) => [binding.client_key, binding.stable_id]));
  assert.deepEqual(oracleContracts.map((/** @type {Record<string,any>} */ contract) => behaviorBindings.get(contract.oracle_contract_client_key)?.slice(0, 5)), ['osc5_', 'osc5_']);
  ...
  const caseSelector = caseWork.available_actions.find((/** @type {Record<string,any>} */ selector) => selector.capability.kind === 'submit_artifact');
  ...
  const caseBindings = new Map(finished.commit_receipt.client_key_bindings.map((/** @type {Record<string,any>} */ binding) => [binding.client_key, binding.stable_id]));
  ...
  assert.deepEqual([...new Set(completeProvenance.nodes.map((/** @type {Record<string,any>} */ node) => node.kind))].sort(), ['atomic_outcome', 'behavior_contract', 'case', 'case_document', 'case_oracle', 'claim', 'fact', 'formal_test_point', 'rendered_output', 'source_unit']);
