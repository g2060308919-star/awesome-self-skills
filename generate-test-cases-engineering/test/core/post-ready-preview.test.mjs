...
function openRequest(state = initial()) {
  const control = nextPreviewControl(ready, state, '0.4.0');
  return {
...
test('open preview creates a bound post-ready presentation without changing ready state', () => {
  ...
  const result = processPreviewRequest({ request, state: initial(), ready, compilerVersion: '0.4.0' });
...
test('exact crash replay returns cached result but same request id with changed content is rejected', () => {
  const request = openRequest();
  const first = processPreviewRequest({ request, state: initial(), ready, compilerVersion: '0.4.0' });
  const replay = processPreviewRequest({ request, state: first.state, ready, compilerVersion: '0.4.0' });
  ...
  const rejected = processPreviewRequest({ request: changed, state: first.state, ready, compilerVersion: '0.4.0' });
...
test('replace invalidates the old presentation and cancel cannot be replayed to revive it', () => {
  ...
  const first = processPreviewRequest({ request: firstRequest, state: initial(), ready, compilerVersion: '0.4.0' });
  const replaceControl = nextPreviewControl(ready, first.state, '0.4.0');
  ...
  const replacement = processPreviewRequest({ request: replacementRequest, state: first.state, ready, compilerVersion: '0.4.0' });
  ...
  const replacedReplay = processPreviewRequest({
    request: firstRequest, state: replacement.state, ready, compilerVersion: '0.4.0'
  });
  assert.equal(replacedReplay.kind, 'rejected');

  const cancelControl = nextPreviewControl(ready, replacement.state, '0.4.0');
  const cancel = {
  ...
  const closed = processPreviewRequest({ request: cancel, state: replacement.state, ready, compilerVersion: '0.4.0' });
  ...
  const staleReplay = processPreviewRequest({ request: firstRequest, state: closed.state, ready, compilerVersion: '0.4.0' });
  assert.equal(staleReplay.kind, 'rejected');

  const reopenedRequest = openRequest(closed.state);
  const reopened = processPreviewRequest({
    request: reopenedRequest, state: closed.state, ready, compilerVersion: '0.4.0'
  });
