...
test('schema registry loads every versioned schema from its manifest', async () => {
  ...
  assert.equal(registry.schemaVersion, '4.2.0');
  assert.equal(registry.compilerVersion, '0.7.0');
...
test('schema integrity mismatch is fatal before an empty run is read', async () => {
  ...
  try {
    ...
    await writeFile(temporaryManifest, originalManifest.replace('"schema_version":"4.2.0"', '"schema_version":"9.0.0"'));
