...
test('schema registry loads every versioned schema from its manifest', async () => {
  ...
  assert.equal(registry.schemaVersion, '4.3.0');
  assert.equal(registry.compilerVersion, '0.8.0');
...
test('schema integrity mismatch is fatal before an empty run is read', async () => {
  ...
  try {
    ...
    await writeFile(temporaryManifest, originalManifest.replace('"schema_version":"4.3.0"', '"schema_version":"9.0.0"'));
...
test('schema tampering is fatal before an empty run is read', async () => {
  ...
  try {
    ...
    await writeFile(tamperedSchema, (await readFile(tamperedSchema, 'utf8')).replace('"3.0.0"', '"9.0.0"'));
    const result = await runCompiler(runDirectory, temporaryRunner);
    const reply = JSON.parse(result.stdout);
