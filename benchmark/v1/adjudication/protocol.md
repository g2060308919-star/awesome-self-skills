...
Any disagreement requires a completed adjudication with the exact `label_key`, both expert values, resolved value, adjudicator, completion time, and rationale. The final label must equal expert agreement or the completed resolution. Corrections create a new `label_version` with `correction_of`; `prior_versions` retains a gap-free chain of complete prior final labels, expert annotations, and adjudications behind verified SHA-256 digests. Every retained version is validated with the same typed label/adjudication rules, so a correction cannot replace history with an empty shell.
