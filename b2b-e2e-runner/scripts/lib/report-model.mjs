...
function buildCaseDetail(testCase, state, caseIndex, evidence) {
...
export function buildReportModel(testCases, executionLog) {
  ...
  const caseDetails = testCases.cases.map((testCase, index) => buildCaseDetail(testCase, executionLog.cases[index], index, evidence));
  ...
  const importantTypes = new Set([
    ...
    "permission_availability", "permission_batch", "permission_wait", "proxy_state", "cleanup_state", "resume_check"
  ]);
  ...
  return {
    run: {
      ...
      status_label: statusLabels[executionLog.run.status] ?? executionLog.run.status,
