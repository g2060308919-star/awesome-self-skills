...
15. For v2, generate only `report.html` through the ledger CLI. Use the returned `chatTableMarkdown` for the full dialogue table; it is not a persisted Markdown report. Validate `snapshotHash`, `eventCount`, `lastSequence`, file content, and the unchanged ledger boundary, scan textual artifacts for secrets, and confirm `reportPath` and `htmlReportPath` both identify the validated HTML. Historical v1 report pairs remain compatibility-only.
...
Give the Run root and four-state counts, then copy the complete five-column `chatTableMarkdown` returned by the validated v2 report command into the final reply. Include every input case exactly once in input order; a report link cannot replace the table. Follow with failed/undetermined highlights, screenshot gaps, proxy verification and automatic proxy cleanup (or “本次未使用代理”), then link the single `report.html`. Never echo a password. All claims must trace to the immutable case snapshot plus execution log.
