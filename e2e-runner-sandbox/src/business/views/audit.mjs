...
export function renderAudit(events) {
  const items = [...events].reverse().map((event) => `<li><span class="timeline-mark" aria-hidden="true"></span><div><strong>${escapeHtml(event.summary)}</strong><small>${escapeHtml(event.time)} · ${escapeHtml(event.actorId)}</small></div></li>`).join("");
